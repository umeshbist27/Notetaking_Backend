...
import { createNote, deleteNote, editNote, extractPublicId, getNotes, uploadImage, } from "../controllers/noteController";
...
describe("Notes - Create", () => {
    ...
    test("create Note throws if req.user is missing", async () => {
        const req = { user: undefined, params: { id: "someId" } };
        const res = {};
        await expect(createNote(req, res)).rejects.toThrow("Unauthorized: No token provided");
    });
    ...
    it("should return 500 and message 'Image upload failed' if upload fails", async () => {
        ...
        jest.spyOn(cloudinary.uploader, "upload_stream").mockImplementation(((options, callback) => {
            const stream = new PassThrough();
            process.nextTick(() => callback(fakeCloudinaryError, undefined));
            return stream;
        }));
        const res = await request
            .post("/api/auth/notes/create")
    ...
    it("should include imageUrl in deletion if it's a Cloudinary URL", async () => {
        jest.spyOn(cloudinary.uploader, "upload_stream").mockImplementationOnce(((options, callback) => {
            const stream = new PassThrough();
            process.nextTick(() => callback(undefined, {
                secure_url: "https://res.cloudinary.com/demo/image/upload/v123/testimg.jpg",
            }));
            return stream;
        }));
        const createRes = await request
...
describe("Notes - Read", () => {
    ...
    test("fetch Note throws if req.user is missing", async () => {
        const req = { user: undefined, params: { id: "someId" } };
        const res = {};
        await expect(getNotes(req, res)).rejects.toThrow("Unauthorized: No token provided");
    });
...
describe("Notes - Update", () => {
    ...
    test("update Note throws if req.user is missing", async () => {
        const req = { user: undefined, params: { id: "someId" } };
        const res = {};
        await expect(editNote(req, res)).rejects.toThrow("Unauthorized: No token provided");
    });
    ...
    it("should mark old imageUrl for deletion when a new one is uploaded", async () => {
        jest.spyOn(cloudinary.uploader, "upload_stream").mockImplementationOnce(((options, callback) => {
            const stream = new PassThrough();
            process.nextTick(() => callback(undefined, {
                secure_url: "https://res.cloudinary.com/demo/image/upload/v123/oldmain.jpg",
            }));
            return stream;
        }));
        const createRes = await request
        ...
        jest.spyOn(cloudinary.uploader, "upload_stream").mockImplementationOnce(((options, callback) => {
            const stream = new PassThrough();
            process.nextTick(() => callback(undefined, {
                secure_url: "https://res.cloudinary.com/demo/image/upload/v123/newmain.jpg",
            }));
            return stream;
        }));
        const res = await request
    ...
    it("should cover extractImageUrls diff logic in editNote", async () => {
        const imageUrl = "https://res.cloudinary.com/demo/image/upload/v123/coverme.jpg";
...
describe("Notes - Delete", () => {
    ...
    test("deleteNote throws if req.user is missing", async () => {
        const req = { user: undefined, params: { id: "someId" } };
        const res = {};
        await expect(deleteNote(req, res)).rejects.toThrow("Unauthorized: No token provided");
    });
...
describe("Notes - Upload Image", () => {
    test("update image throws if req.user is missing", async () => {
        ...
        const res = {};
        const next = jest.fn();
        await expect(uploadImage(req, res, next)).rejects.toThrow("Unauthorized: No token provided");
    });
    ...
    it("should handle uploadToCloudinary throwing non-Error value", async () => {
        jest.spyOn(cloudinary.uploader, "upload_stream").mockImplementation(((options, callback) => {
            const stream = new PassThrough();
            process.nextTick(() => callback("Non-Error Failure"));
            return stream;
        }));
        const res = await request
            .post("/api/auth/notes/upload-image")
...
describe("Notes - Helper to extractPublic id", () => {
    it("should extract public ID correctly and fallback to empty string", () => {
        expect(extractPublicId("https://res.cloudinary.com/demo/image/upload/v123/abc.jpg")).toBe("abc");
